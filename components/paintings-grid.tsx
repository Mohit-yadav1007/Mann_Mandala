      ...
      <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 xs:gap-3 sm:gap-4 md:gap-6">
        {paintings.map((painting, idx) => (
          <button
            ...
            className="group text-left rounded-xl border border-amber-100 bg-white transition-all hover:shadow-lg hover:-translate-y-1 animate-in-up focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-opacity-50"
          ...
          >
            ...
            <div className="p-2 sm:p-3">
              <h3 className="text-sm sm:text-base font-medium text-amber-800 line-clamp-1">{painting.title}</h3>
              <p className="text-[10px] sm:text-xs text-amber-700/80 line-clamp-2">{painting.caption}</p>
            </div>
          </button>
        ))}
      </div>
