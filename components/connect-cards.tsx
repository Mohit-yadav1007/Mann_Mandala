      ...
      {cards.map((c, i) => {
        ...
        return (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`group rounded-xl border p-3 sm:p-4 transition-all duration-300 ${v.wrapper} ${v.ring} animate-in-up hover-raise hover-tilt touch-target`}
            style={{ animationDelay: `${i * 70}ms` }}
          >
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-2 text-sm sm:text-base font-medium text-slate-900">
            ...
            {c.note && <p className="mt-1 text-xs sm:text-sm text-slate-700 leading-relaxed">{c.note}</p>}
          </a>
        )
      })}
    </div>
