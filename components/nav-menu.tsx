  ...
  return (
    <div className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-sm border-b border-amber-100 shadow-sm">
      <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
        <Link href="/" className="font-semibold text-amber-600 text-lg flex items-center gap-2">
          <span className="h-8 w-8 rounded-full bg-amber-600/20 flex items-center justify-center text-amber-600 font-bold">M</span>
          <span>Mann Mandalas</span>
        </Link>
        ...
        {/* Mobile menu button */}
        <button 
          className="md:hidden p-2 rounded-md text-amber-800 hover:bg-amber-50 focus:outline-none"
          ...
          <nav className="flex flex-col py-2">
            {navLinks.map((link) => (
              <Link 
                key={link.label} 
                href={link.href}
                className="px-4 py-3 text-amber-800 hover:bg-amber-50 transition-colors"
